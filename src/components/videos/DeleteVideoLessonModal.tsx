...
interface DeleteVideoLessonModalProps {
  isOpen: boolean
...
export function DeleteVideoLessonModal({
  isOpen,
  lesson,
  onClose,
  onConfirm,
}: DeleteVideoLessonModalProps) {
  ...
  async function handleConfirmDelete() {
    ...
    try {
      await onConfirm()
      showSnackbar('Video lesson deleted successfully')
      onClose()
    } catch {
      const errorMessage = 'Failed to delete video lesson. Please try again.'
  ...
  const lessonLabel = lesson.lessonName.trim() || 'this lesson'
  ...
  return (
    ...
    >
      <button
        type="button"
        aria-label="Close delete video lesson dialog"
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="delete-video-lesson-modal-title"
      ...
      >
        <div className="flex items-start justify-between border-b border-border-subtle bg-surface px-gutter py-5">
          <div className="flex flex-col gap-2 pr-4">
            <h2 id="delete-video-lesson-modal-title" className="text-h3 text-on-surface">
              Delete Video Lesson
            </h2>
        ...
        <div className="px-gutter py-gutter">
          <p className="text-body-md text-on-surface">
            ...
            <span className="font-semibold">{lessonLabel}</span> from module{' '}
            <span className="font-semibold">{lesson.moduleName || '—'}</span>?
          </p>
          {error ? (
