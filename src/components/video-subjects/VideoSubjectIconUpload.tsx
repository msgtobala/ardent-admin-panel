...
import { validateVideoSubjectIconFile } from '@/lib/video-subject-icon-storage'
import { MaterialIcon } from '@/components/ui/MaterialIcon'
import { VideoSubjectIcon } from './VideoSubjectIcon'

interface VideoSubjectIconUploadProps {
  subjectName: string
  file: File | null
...
export function VideoSubjectIconUpload({
  subjectName,
  file,
  previewUrl,
  error,
  ...
  function handleFileSelected(selectedFile: File | null) {
    ...
    const validationError = validateVideoSubjectIconFile(selectedFile)
    if (validationError) {
  ...
  return (
    <div className="flex w-full flex-col gap-1">
      <span className="text-label-sm font-semibold text-on-surface">Upload Icon</span>
      <div
        role="button"
        tabIndex={disabled ? -1 : 0}
        aria-label="Upload video subject icon"
        onClick={handleClick}
